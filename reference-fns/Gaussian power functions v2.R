...
ctsClus_getClusterSize_clusterNum <- function (clusterNum.list){
  
  clusterSize.seq = seq(1, 200, by = 4)
  f = function(y){ 
    power.list = sapply(clusterSize.seq, 
                        function(x){helper_power(eff.size = effsize, cluster.num=y, cluster.size=x )} ) 
    check = power.list > power
    if ( sum(check)>=1 ){ clusterSize.seq[min(which(check))]} else{NA}
  }
  sapply(clusterNum.list, function(x){f(x)} )
...
### Option 6 ###
# Power is fixed
# Effect size if fixed
# Cluster size as a function of cluster number
power = 0.8
effsize = 0.8
ClusterNums = seq(2, 100, by = 4)
fit = ctsClus_getClusterSize_clusterNum (ClusterNums )
plot (fit~ClusterNums, ylab = "Cluster Size", xlab = "Cluster Number", type = "l", col = 4)
